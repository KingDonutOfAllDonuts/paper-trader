...
const BuyStockPopup = ({ onSubmit, isOpen, symbol, onClose, accName, cashBalance}) => {
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
      <div className="bg-white rounded-lg p-6 w-96 relative">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium mb-2">
            Cash available to trade: {formatNumber(cashBalance)}
            </label>
          </div>
          ...
          <div className="mb-4">
            <p className="text-sm font-medium">Estimated Cost: {stockPrice == -1 ?  'Loading...': formatNumber(totalCost)}</p>
          </div>
          ...
          >
            {stockPrice == -1 ? <LoadingDots/> : 'Buy Stocks'}
          </button>
        </form>
      </div>
